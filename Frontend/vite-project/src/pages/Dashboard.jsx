...
export default function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async (isBackground = false) => {
    ...
    try {
      ...
      if (payableLoans.length > 0) {
        const repaymentPromises = payableLoans.map((loan) =>
          loanApi.getById(loan.id),
        );
  ...
  const handleFlexiblePayment = async () => {
    ...
    try {
      ...
      toast({
        title: "STK Push Sent",
        description: "Your balance will update once confirmed.",
      });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 pt-24 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <Button
            ...
            className="shadow-lg hover:scale-105 transition-all bg-emerald-700 hover:bg-emerald-800"
          >
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-none shadow-md">
              <CardHeader className="flex flex-row items-center justify-between border-b pb-4 mb-4">
                ...
                <Badge
                  variant="secondary"
                  className="px-3 bg-emerald-50 text-emerald-700 border-emerald-100"
                >
                ...
                {userLoans.length > 0 ? (
                  userLoans.map((loan) => {
                    ...
                    return (
                      <div
                        key={loan.id}
                        className="p-5 border rounded-3xl bg-white shadow-sm hover:shadow-md transition-all"
                      >
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <Badge
                            className={`uppercase ${
                              loan.status?.toUpperCase() === "REPAYING"
                                ? "bg-emerald-600"
                                : "bg-slate-500"
                            }`}
                          >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {p.status?.toUpperCase() === "PENDING" &&
                          isLoanPayable(
                            userLoans.find((l) => l.id === p.loanId)?.status,
                          ) && (
                            <Button
                              size="sm"
                              className="h-8 text-[10px] bg-emerald-700"
          ...
          <div className="space-y-6">
            <Card className="border-none shadow-md">
              <CardHeader>
                <CardTitle className="text-emerald-900">Credit Score</CardTitle>
              </CardHeader>
              <CardContent className="flex flex-col items-center pb-8">
                <CreditScoreGauge score={user.creditScore || 650} size="lg" />
                <p className="text-xs text-center text-muted-foreground mt-6 px-4 italic">
                  Healthy repayment habits lead to higher limits.
                </p>
              </CardContent>
            </Card>
            <Card className="border-none shadow-md">
              ...
              <CardContent className="space-y-2">
                <Button
                  variant="outline"
                  className="w-full justify-start border-emerald-100 text-emerald-700 hover:bg-emerald-50"
                  onClick={() => navigate("/apply")}
                >
      ...
      <Dialog open={!!selectedLoan} onOpenChange={() => setSelectedLoan(null)}>
        <DialogContent className="rounded-3xl">
          <DialogHeader>
            <DialogTitle className="text-emerald-900 text-xl font-bold">
              Loan Overview
            </DialogTitle>
          </DialogHeader>
          {selectedLoan && (
            <div className="grid grid-cols-2 gap-4 mt-4 bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100">
              ...
              <div>
                ...
                <p className="font-bold text-lg text-emerald-700">
                  {formatCurrency(selectedLoan.totalRepaid || 0)}
                </p>
              </div>
              <div>
                <p className="text-xs text-emerald-600 font-semibold uppercase">
                ...
                <Badge className="bg-emerald-600">{selectedLoan.status}</Badge>
              </div>
              <div>
                <p className="text-xs text-emerald-600 font-semibold uppercase">
                  Interest
      ...
      <Dialog
        open={!!activeFlexibleLoan}
        onOpenChange={() => setActiveFlexibleLoan(null)}
      >
        <DialogContent className="rounded-3xl">
          <DialogHeader>
            <DialogTitle className="text-emerald-900 text-xl font-bold">
          ...
          {activeFlexibleLoan && (
            <div className="mt-4 space-y-4">
              <p className="text-xs text-slate-500">
                Enter custom amount to pay towards loan ending in ...
                {activeFlexibleLoan.id.slice(-4)}
              </p>
              <div className="relative">
                <span className="absolute left-4 top-2.5 font-bold text-emerald-700">
                  KSh
                </span>
                <input
                  type="number"
                  className="w-full border-2 border-emerald-100 rounded-2xl px-12 py-3 focus:border-emerald-500 outline-none text-lg font-bold"
                  placeholder="0"
                  value={flexiblePaymentAmount}
                  onChange={(e) => setFlexiblePaymentAmount(e.target.value)}
                />
              </div>
              <Button
                className="w-full h-12 bg-emerald-700 hover:bg-emerald-800 text-lg rounded-2xl"
                onClick={handleFlexiblePayment}
                disabled={processingPaymentId === activeFlexibleLoan.id}
              >
                {processingPaymentId === activeFlexibleLoan.id ? (
